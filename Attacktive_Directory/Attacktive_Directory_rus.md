...
### Получены флаги, подтверждающие компрометацию системы.
В ходе тестирования была успешно выполнена компрометация инфраструктуры Active Directory через атаку AS-REP Roasting, последующее перечисление SMB-ресурсов и получение учётных данных backup-аккаунта. Использование DRSUAPI позволило извлечь NTLM-хэши доменных пользователей, включая Administrator, что привело к полной компрометации контроллера домена.
